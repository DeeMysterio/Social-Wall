...
export default {
	...
	data() {
		return {
			...
			comments: [],
			show_comments: false,
			is_globally_pinnable: frappe.user_roles.includes('System Manager') && social_wall.social.is_home_page(),
			is_pinnable: false,
	...
	computed: {
		...
		is_pinned() {
			return false && social_wall.social.is_profile_page(this.post.owner)
				|| this.post.is_globally_pinned && social_wall.social.is_home_page()
		},
		options() {
			const options = []
			if (this.can_pin) {
	...
	methods: {
		...
		toggle_like() {
			frappe.xcall('social_wall.social_wall.doctype.post.post.toggle_like', {
		...
		delete_post() {
			frappe.confirm(__("Are you sure you want to delete this post?"), () => {
				frappe.dom.freeze();
				frappe.xcall('social_wall.social_wall.doctype.post.post.delete_post', {
					'post_name': this.post.name
				}).then(frappe.dom.unfreeze)
			})
		},
		update_seen() {
			frappe.xcall('social_wall.social_wall.doctype.post.post.set_seen', {
				post_name: this.post.name
			}).then(() => this.post.seen = true)
		},
		generate_preview(link_element) {
			// TODO: move the code to separate component
			frappe.xcall('social_wall.social_wall.doctype.post.post.get_link_info', {
